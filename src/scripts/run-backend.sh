...
(cd "$BACKEND_DIR/mcp" && uv run python -m uvicorn main:rest_app --host 0.0.0.0 --port 8001 --reload) &
MCP_PID=$!

# Start A2A agent (port 5002)
echo "Starting A2A agent on port 5002..."
(cd "$BACKEND_DIR/agent-a2a" && uv run python -m uvicorn main:app --host 0.0.0.0 --port 5002 --reload) &
A2A_PID=$!
...
(cd "$BACKEND_DIR/api" && uv run python -m uvicorn main:app --host 0.0.0.0 --port 8000 --reload) &
API_PID=$!
