...
async function proxyToBackend(req: NextRequest, sessionId: string) {
  ...
  const upstream = await fetch(
    `${AGENT_API_BASE_URL}/api/sessions/${encodeURIComponent(sessionId)}`,
    {
      method: req.method,
      headers,
      body,
...
export async function GET(req: NextRequest, context: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to reach backend session endpoint' },
      { status: 502 },
    );
  }
}
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ sessionId: string }> }) {
  if (!isAuthEnabled) {
    return NextResponse.json({ error: 'Session APIs are disabled' }, { status: 404 });
  }
