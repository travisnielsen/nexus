...
def validate_token(token: str) -> dict[str, Any]:
    ...
    payload = jwt.decode(
        token,
        public_key,  # pyright: ignore[reportArgumentType]
        algorithms=["RS256"],
...
class EntraIDAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not AUTH_ENABLED:
            logger.debug(
                "Authentication not enabled - allowing unauthenticated request"
            )
            return await call_next(request)
        ...
        if not AZURE_AD_TENANT_ID or not AZURE_AD_CLIENT_ID:
            logger.warning(
                "AUTH_ENABLED=true but Azure AD not configured - allowing request without validation"
            )
            return await call_next(request)
        ...
        try:
            ...
            request.state.user_name = payload.get("name") or payload.get(
                "preferred_username"
            )
            ...
            logger.info(
                f"ACCESS: {request.method} {path} | app={appid} upn={upn} name={name}"
            )
