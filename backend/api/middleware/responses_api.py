...
_current_agui_thread_id: ContextVar[str | None] = ContextVar(
    "current_agui_thread_id", default=None
)
...
class ResponsesApiThreadMiddleware(ChatMiddleware):
    ...
    async def process(
        ...
        if agui_thread_id and HAS_OTEL:
            ...
            if current_span and current_span.is_recording():
                ...
                logger.debug(
                    "[ResponsesApiThreadMiddleware] Set span attribute conversation_id: %s",
                    agui_thread_id,
                )

        # Log incoming messages for debugging
        logger.debug(
            "[ResponsesApiThreadMiddleware] Incoming messages: %d",
        ...
        for i, msg in enumerate(context.messages or []):
                ...
                for c in (
                    ...
                    if hasattr(c, "call_id"):
                        contents_info.append(
                            f"{c_type}(call_id={c.call_id[:12] if c.call_id else None})"
                        )
                    elif hasattr(c, "name"):
            ...
            logger.debug(
                "[ResponsesApiThreadMiddleware]   msg[%d]: role=%s, contents=%s",
                i,
                role,
                contents_info,
            )
        ...
        if agui_thread_id and agui_thread_id in _thread_response_store:
            ...
            logger.info(
                "[ResponsesApiThreadMiddleware] CONTINUING thread %s with response_id %s",
                agui_thread_id,
                stored_response_id,
            )
            ...
            logger.info(
                "[ResponsesApiThreadMiddleware] NEW/FRESH conversation for thread %s",
                agui_thread_id,
            )
            if context.options is None:
                context.options = {}
        ...
        logger.debug(
            "[ResponsesApiThreadMiddleware] After filtering: %d messages",
            len(context.messages) if context.messages else 0,
        )
        ...
        if context.is_streaming and context.result is not None:  # pyright: ignore[reportAttributeAccessIssue]
            ...
            if new_response_id and new_response_id.startswith(("resp_", "conv_")):
                ...
                logger.debug(
                    "[ResponsesApiThreadMiddleware] Stored response_id %s for thread %s",
                    new_response_id,
                    agui_thread_id,
                )
    ...
    async def _capture_response_id(
                ...
                for content in update.contents:
                    if _is_function_call(content):
                        ...
                        if tool_name:
                            ...
                            logger.debug(
                                "[ResponsesApiThreadMiddleware] Saw tool call: %s",
                                tool_name,
                            )
        ...
        ended_with_frontend = (
            last_tool_name in FRONTEND_ONLY_TOOLS if last_tool_name else False
        )
        ...
        if ended_with_frontend:
            ...
            logger.info(
                "[ResponsesApiThreadMiddleware] Response ended with frontend tool '%s' - STORING response_id for continuation",
                last_tool_name,
            )
            if agui_thread_id and last_response_id:
                if last_response_id.startswith(("resp_", "conv_")):
                    ...
                    logger.info(
                        "[ResponsesApiThreadMiddleware] Stored response_id %s for thread %s (awaiting tool result)",
                        last_response_id,
                        agui_thread_id,
                    )
    ...
    def _filter_messages_for_fresh_start(self, context: ChatContext) -> None:
        ...
        logger.debug(
            "[ResponsesApiThreadMiddleware] _filter_messages_for_fresh_start: examining %d messages",
            original_count,
        )
        ...
        for i, msg in enumerate(messages):
            ...
            if msg_contents:
                for c in (
                    msg_contents if isinstance(msg_contents, list) else [msg_contents]
                ):
                    c_type = type(c).__name__
            ...
            if role == "assistant":
                ...
                if msg_contents:
                    contents = (
                        msg_contents
                        if isinstance(msg_contents, list)
                        else [msg_contents]
                    )
                    for c in contents:
                ...
                if has_tool_related:
                    logger.debug(
                        "[ResponsesApiThreadMiddleware]     -> REMOVE (assistant with tool call/result)"
                    )
                    continue

                # Keep pure text assistant messages
                logger.debug(
                    "[ResponsesApiThreadMiddleware]     -> KEEP (pure text assistant)"
                )
                filtered.append(msg)
                continue
