...
# Track if Assistants API patch was applied
_assistants_patch_applied = False
_original_assistants_create_event_attributes = None


def apply_assistants_conversation_id_patch() -> bool:
    """Patch Assistants API instrumentor to include gen_ai.conversation.id.
    
    The Assistants API instrumentor (_AIAgentsInstrumentorPreview) sets gen_ai.thread.id
    with the Azure thread ID, but doesn't set gen_ai.conversation.id. This patch adds
    the CopilotKit threadId as gen_ai.conversation.id for consistent querying across
    both Responses and Assistants API modes.
    
    Returns:
        True if patch was applied, False otherwise.
    """
    global _assistants_patch_applied, _original_assistants_create_event_attributes
    
    if _assistants_patch_applied:
        logger.debug("[ASSISTANTS-PATCH] Already applied, skipping")
        return True
    ...
    try:
        ...
        try:
            ...
            logger.debug("[ASSISTANTS-PATCH] Azure SDK Assistants telemetry not available, skipping patch")
            return False
        
        # Save original method
        _original_assistants_create_event_attributes = _AIAgentsInstrumentorPreview._create_event_attributes  # type: ignore[attr-defined]
        ...
        def patched_create_event_attributes(
            ...
            cancelled_at=None,
            failed_at=None,
            run_step_last_error=None,
            usage=None,
        ):
            """Patched _create_event_attributes that adds gen_ai.conversation.id.
            
            The original method sets gen_ai.thread.id with the Azure thread ID.
            We also add gen_ai.conversation.id with the CopilotKit threadId for
            consistent querying across API modes.
            """
            # Call original to get base attributes
            attrs = _original_assistants_create_event_attributes(
                self,
                thread_id=thread_id,
                agent_id=agent_id,
            ...
            # Add gen_ai.conversation.id from CopilotKit threadId
            copilotkit_thread_id = get_current_agui_thread_id().get()
            if copilotkit_thread_id:
                attrs[GEN_AI_CONVERSATION_ID] = copilotkit_thread_id
                logger.debug("[ASSISTANTS-PATCH] Added conversation_id=%s (Azure thread_id=%s)", 
                           copilotkit_thread_id, thread_id)
            
            return attrs
        
        # Apply the patch
        _AIAgentsInstrumentorPreview._create_event_attributes = patched_create_event_attributes  # type: ignore[attr-defined]
        
        _assistants_patch_applied = True
        logger.info("[ASSISTANTS-PATCH] Patched Assistants API for conversation_id injection")
        return True
        
    except ImportError as e:
        logger.debug("[ASSISTANTS-PATCH] Could not import required modules: %s", e)
        return False
    except Exception as e:
        logger.warning("[ASSISTANTS-PATCH] Failed to apply patch: %s", e)
        return False
